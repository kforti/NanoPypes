...
@click.option('-s', '--data_splits', 'data_splits', multiple=True, help='the number of splits to divide raw minion sequence batches into. Start with the number of workers you intend to use and optimize from there', required=True, type=int)
@click.option('-b', '--batch_bunches', 'batch_bunches', multiple=True, help="The number of batches to process at one time.", type=int)
@click.option('-c', '--continue_on', 'continue_on', multiple=True, help="if True then the basecaller will continue from it's previous start location.", type=bool)
